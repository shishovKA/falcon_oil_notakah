...
class Config {
	constructor() {
		const configText = this.readFile()
		...
		this.timeTankerDelivery = this.timeTankerDelivery / 2
		this.year = 12
	}
...
class GasMain extends Gas {
	...
	buildWorkPlace() {
		...
		const maxStations = Math.ceil(maxFuel / (config.amountCarMonth * config.amountCarMonth))
